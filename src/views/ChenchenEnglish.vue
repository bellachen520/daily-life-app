...
async function load() {
  ...
  records.value = (await getByMonth(y, m)).filter(r => r.type === 'english')
  streak.value = await getStreak('english')
}
...
async function submit() {
  ...
  try {
    await addCheckin({ ...form.value, type: 'english' })
    showForm.value = false
    form.value = { type: 'english', date: today, duration: 30, content: '', notes: '' }
    load()
  } catch (e: any) {
    // 同类型同天已打卡
...
<template>
  <div>
    <van-nav-bar title="英语学习打卡" left-arrow @click-left="goBack" />
    <div class="page-container">
      ...
      <div class="section">
        <StreakBadge :streak="streak" label="英语" />
      </div>
      ...
      <div v-if="records.length === 0">
        <EmptyState text="本月还没有英语打卡" description="点击下方按钮开始打卡" />
      </div>
    ...
    <van-popup v-model:show="showForm" position="bottom" round :style="{ padding: '20px' }">
      <h3 style="margin-bottom: 16px; font-size: 18px;">英语学习打卡</h3>
      <van-field v-model="form.date" label="日期" placeholder="YYYY-MM-DD" />
      <van-field v-model="form.content" label="学习内容" type="textarea" rows="2" placeholder="今天学了什么？" />
      <van-field v-model="form.duration" label="时长(分)" type="number" placeholder="学习时长" />
